...
export function RosterPage() {
  ...
  const { weekStart, weekEnd, nextWeek, prevWeek, currentWeekShifts, addShift, deleteShift } = useRoster(departmentId);
            ...
            <div key={dateStr} className={`${styles.dayCol}${todayDay ? ` ${styles.today}` : ''}`}>
              ...
              <div className={styles.dayBody}>
                {dayShifts.length === 0 ? (
                  ...
                  dayShifts.map((sb) => (
                    <ShiftBlock key={sb.shift.id} shift={sb.shift} user={sb.user} onDelete={deleteShift} />
                  ))
                )}
              </div>
            </div>
